...
import pandas as pd
from user_profile import UserProfile, ProfileCollector
...
def save_to_excel(profile: UserProfile, plan_text: str, timeline_df: pd.DataFrame, 
                  excel_path, thread_id: str = "XXX", human_feedback: str = "") -> int:
    ...
    export_latest_plan_to_ics(excel_path)
...
def export_latest_plan_to_ics(excel_path: str, output_file: str = "plan.ics"):
    """
    Export the newest plan from sheet 'plan_timeline' to an .ics calendar file
    """
    ...
    df = pd.read_excel(excel_path, sheet_name="plan_timeline")
    ...
    # Get newest log_id
    newest_log_id = df["log_id"].max()

    # Filter newest plan
    plan_df = df[df["log_id"] == newest_log_id].copy()
    ...
    for _, row in plan_df.iterrows():
        ...
        end_dt = datetime.strptime(
            f"{row['date']} {row['end_time']}", "%Y-%m-%d %H:%M"
        )
        ...
        e.begin = start_dt
        e.end = end_dt
        e.description = f"Category: {row['category']}"
    ...
    print(f"\n📅 Calendar exported: {output_file}")
    print(f"   Plan ID: {newest_log_id}")
    print(f"   Events: {len(plan_df)}")


if __name__ == "__main__":
    data = {
        "task": ["Gym", "Study AI", "Read"],
        "start_time": ["2024-06-01 09:00", "2024-06-01 10:30", "2024-06-01 12:00"],
        "end_time": ["2024-06-01 10:00", "2024-06-01 12:00", "2024-06-01 13:00"],
    }
    timeline_df = pd.DataFrame(data)
    export_latest_plan_to_ics("planning_data.xlsx", "plan.ics")
